...
def get_meta_data(
    ...
    for key, row_keys in META_MAP.items():
        ...
        if not found:
            ...
            except_files_for_reference = [
                'バリエーション_train1_20240523_takayanagi_20240523_アノテーターコード2_3.xlsx',
                '創作データ3_書物・映画_20240520_saito_20240524.xlsx',
            ]
            #raise ValueError(f'No {row_keys}')
            if key == 'output-reference' and any(path.endswith(x) for x in except_files_for_reference):
                meta[key] = []
            else:
                raise ValueError(f'No {row_keys}')
    new_output_type = []
    ...
    with open(output_file, 'w', encoding='utf-8') as f_output:
        for i, row in tqdm(
            ...
            try:
                ...
                meta = get_meta_data(row, input_path, skip=True)
                ...
                if a_id is None:
                    ...
                    except_files = [
                        #'tmp/single_20240525/trainingData/F7.1.10-13_original_教師用.xlsx',
                        'tmp/single_20240525/trainingData/インターグループ_教師用データ_20240524.xlsx',
                        'F7.1.10-13_original_教師用_20240530.xlsx',
                    ]
                    #except_ids = [
                    #    (1015, 1),
                    #    (1016, 1),
                    #    (1017, 1),
                    #]
                    #logger.debug('q_id: %s, a_id: %s', q_id, a_id+1)
                    #logger.debug(
                    #    '(%s, %s) in %s: %s',
                    #    q_id, a_id+1, except_ids, (q_id, a_id+1) in except_ids,
                    #)
                    #if (q_id, a_id+1) in except_ids:
                    #if answers[a_id]['file'] in except_files:
                    if any(answers[a_id]['file'].endswith(x) for x in except_files):
                        continue
                        #pass
                    else:
